...
def load_data():
    ...
    # create new dataset where nodes are only mapped to one distribution network node
    temp_data = []
    node_count = 1
    while node_count < max([element[2] for element in data]):
        for dist_nodes in data:
            if dist_nodes[2] == node_count:
                temp_data.append(dist_nodes)
                break
        node_count += 1
    ...
    with open(r'data/congestion_weekday.csv', 'r') as nodecsv: # Open the file
        nodereader = csv.reader(nodecsv) # Read the csv
        congestion_weekday = [n for n in nodereader][2:]

    with open(r'data/congestion_weekend.csv', 'r') as nodecsv: # Open the file
        nodereader = csv.reader(nodecsv) # Read the csv
        congestion_weekend = [n for n in nodereader][2:]

    # build congestion data
    congestion = []
    for i in range(len(congestion_weekday)):
        congestion_weekend[i] = congestion_weekend[i][0:6]
        congestion_weekday[i] = congestion_weekday[i][0:6]
        congestion.append([int(congestion_weekday[i][0]), congestion_weekday[i][1], congestion_weekday[i][2:6], congestion_weekend[i][2:6]])
...
def bayesian_build(dataset):
    # VSF distribution
    low_threshold = .1 # from the reference paper
    high_threshold = .2 # from the reference paper
    ...
    d_low_t = 0.3
    d_high_t = 0.7 # made up values, not sure of the precise ones
    ...
    # probability solely considering area and not day/time. this would require the construction of a nested bayesian network
    for node in dataset:
    ...
    candidate_vals = [low_threshold, high_threshold, congestion_probabilities, base_prob, d_low_t, d_high_t]

    return model, candidate_vals

def determine_candidates(model, dataset, candidate_vals):
    # DETERMINE CANDIDATES
    candidates = []
    low_threshold = candidate_vals[0]
    high_threshold = candidate_vals[1]
    congestion_probabilities = candidate_vals[2]
    base_prob = candidate_vals[3]
    d_low_t = candidate_vals[4]
    d_high_t = candidate_vals[5]
...
def candidate_process():
    dataset = load_data()
    model, candidate_vals = bayesian_build(dataset)
    candidates = determine_candidates(model, dataset, candidate_vals)
